...
```text
DOCUMENT_STATUS=BUILDER_ADR_CANDIDATE_CATALOG_REMEDIATED_FOR_RETEST_04
MISSION=LEA-18
LINEAR_ISSUE=LEA-18
PULL_REQUEST=40
...
| ADR-CAND-008 | P0 | arquitetura dos motores A–H e envelope de análise | snapshot imutável, inputs explícitos, ausência de efeito colateral não declarado | interfaces, paralelismo, determinismo, cache e tratamento de contradições | antes do Documento Mestre final |
...
| ADR-CAND-011 | P0 | contrato e isolamento dos adaptadores `NULL|SIMULATED|CONTROLLED_UI|LIVE_GATED` | Modo A autorizado; Modo B desligado até todos os gates LIVE | plataformas, allowlists, identidade, payload, termos, jurisdição, elegibilidade, limites e testes negativos | antes do Documento Mestre final |
| ADR-CAND-012 | P0 | topologia e autoridade do kill switch | kill switch domina fila, grant, retry e dispatch em qualquer modo | local/remoto, latência, disponibilidade, autenticação, auditoria e recovery | antes do Documento Mestre final |
...
| ADR-CAND-018 | P1 | estratégia de testes e evidência por camada | especificação não equivale a runtime; provas negativas e gates LIVE verificáveis | unit, property, contract, replay, E2E controlado, crash/restart e relatórios | antes do plano de implementação aprovado |
...
| execução controlada e LIVE gated | ADR-CAND-011, 013, 014 |
...
## 6. Política A+B e escopo da missão

```text
MODE_A_POLICY=AUTHORIZED
MODE_B_ARCHITECTURAL_SUPPORT=AUTHORIZED
MODE_B_RUNTIME_ACTIVATION=OUT_OF_SCOPE_FOR_LEA_18
MODE_B_DEFAULT=DISABLED
LIVE_FINANCIAL_ADAPTER=SUPPORTED_BY_SEPARATE_LIVE_GATE
PRODUCTION_BROKER_INTEGRATION=REQUIRES_SEPARATE_MISSION_AND_GATE
REAL_MONEY_OPERATION=DISABLED_UNTIL_ALL_LIVE_GATES_PASS
PRODUCTION_CREDENTIAL_MODEL=REQUIRES_SEPARATE_SECURITY_DECISION
AUTONOMOUS_EXTERNAL_INFRASTRUCTURE=OUT_OF_SCOPE_FOR_LEA_18
```

Esses itens não são proibidos arquiteturalmente de forma absoluta. Eles não serão implementados nem ativados pela LEA-18. Qualquer ativação futura exige missão própria, decisão comercial e legal, conformidade com termos e jurisdição da plataforma, elegibilidade do titular, threat model, autorização explícita e revisão crítica independente.
...
MODE_B_DECISION_SCOPE_RECORDED=PASS
POLICY_A_B_ALIGNMENT=PASS_BUILDER
ADR_CREATED=NO
